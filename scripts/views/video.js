...
export const getVideoView = ({id, src, title, likes}) => {
    ...
    // alt videos / !!! don't forget onclick
    const NodeStringified = `
    <article>
        <a href="javascript:lightbox.open(${id})">
            <video alt="${title} aria-label="${title} video, open closeup view" src=${src}></video>
        </a>
        <div class="mediaInfos">
            ...
            <div>
                ...
                src="assets/icons/heart.svg" onmouseover="this.src='assets/icons/heartactive.svg'" onmouseout="this.src='assets/icons/heart.svg'">
            </div>
        </div>
    </article>
    `
...
export const getShortVideoView = ({id, src, title}) => {
    ...
    <div class="arrownmedia-container">
        ...
        <article class="light-container" id="media-${id}">
            <video tabindex="1" id="main-media" src=${src} alt="${title}" aria-label="${title}" controls></video>
            <h2>${title}</h2>
        </article>
        <div class="rightarrow-container">
            ...
            alt="close lightbox" src="assets/icons/closegallery.svg" onclick="lightbox.close()"/>
        </div>
    </div>
    `
    return StringtoNode(NodeStringified)
